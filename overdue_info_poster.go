...
func main() {
	...
	for i := 0; i < len(infos); i++ {
		...
		text := fmt.Sprintf("hi @%s 同学，你借阅 @%s 大牛 的书《%s》到期了，%s! 图书圈：%s", infos[i].Borrower.Name, infos[i].Owner.Name, infos[i].BookInfo.Name, msgs[r.Intn(len(msgs))], url)
		print(text)
		print("some thing =====")
